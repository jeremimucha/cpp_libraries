...
public:
    ...
    // based protocols such as HTTP, you want this to be pretty large,
    // like 64kb. For packet based protocols, then it will be much smaller,
    ...
    // Starts and a/synchronous connect.
    ...
    // Posts an asynchronous disconnect event for the object process.
    void Disconnect();
}; // Connection
/* ------------------------------------------------------------------------- */
...
private:
    ...
    // called on an a ddress that is not available.
    virtual void OnError( const boost::system::error_code& error ) = 0;
...
public:
    ...
    // Retunrs true if this object has an error associated with it.
    bool HasError();
...
public:
    ...
    // until the networking system is stopped, so do not call on single
    ...
    // networking interactions will be possible afterwards until Reset i called.
    void Stop();

    // Restarts the networking system after Stop has been called. A new work
